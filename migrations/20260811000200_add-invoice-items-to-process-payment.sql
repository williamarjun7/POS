-- ════════════════════════════════════════════════════════════════════════════
-- MIGRATION: Add invoice_items insertion to process_payment RPC
-- ════════════════════════════════════════════════════════════════════════════
--
-- Problem:
--   Invoice line items are inserted client-side (POS.tsx → insertInvoiceItems)
--   AFTER the process_payment RPC succeeds. If the client-side insert fails
--   (network timeout, 403, navigation away), the invoice is created with
--   zero line items — showing "0 items" on the Finance page.
--
-- Fix:
--   Add a p_invoice_items JSONB parameter to process_payment and insert
--   the items inside the function, making them ATOMIC with the invoice.
--   The client-side fallback insert is kept for backward compatibility
--   (older cached pages that don't pass items), but the primary path is
--   now inside the RPC.
--
-- The p_invoice_items parameter accepts a JSON array of objects:
--   [{ "name": "Item Name", "quantity": 2, "unit_price": 150.00 }]
-- Each row's menu_item_id and total_price are computed inside the function.
-- ════════════════════════════════════════════════════════════════════════════
...
CREATE OR REPLACE FUNCTION public.process_payment(
  ...
  p_invoice_items           JSONB DEFAULT '[]'::jsonb  -- NEW: atomic invoice items
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- This eliminates the race condition where invoice_items were inserted
  -- client-side after the RPC and could fail silently.
