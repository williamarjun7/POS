-- ════════════════════════════════════════════════════════════════════════════
-- MIGRATION: Update process_payment RPC to accept serving_type & packaging_fee
-- ════════════════════════════════════════════════════════════════════════════
--
-- Problem:
--   The p_invoice_items parameter only supported name, quantity, and unit_price.
--   The new invoice_items.serving_type and invoice_items.packaging_fee columns
--   were never populated by the RPC.
--
-- Fix:
--   Update the p_invoice_items JSONB processing loop to extract serving_type
--   and packaging_fee from each item object and store them in the invoice_items
--   row. Old items without these fields default to 'dine_in' and 0.
--
-- The p_invoice_items parameter now accepts:
--   [{ "name": "Espresso", "quantity": 2, "unit_price": 100,
--      "serving_type": "takeaway", "packaging_fee": 10 }]
--
-- Backward compatibility:
--   ✓ Old records without serving_type/packaging_fee default to 'dine_in'/0
--   ✓ Existing callers that don't pass these fields continue to work
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- STEP 3: Insert invoice_items (ATOMIC — inside the RPC, not client-side)
  -- Now also stores serving_type and packaging_fee from the JSONB items.
  IF jsonb_array_length(p_invoice_items) > 0 THEN
    FOR v_item IN SELECT * FROM jsonb_array_elements(p_invoice_items)
    LOOP
      INSERT INTO invoice_items (invoice_id, name, quantity, unit_price, total_price, serving_type, packaging_fee)
      VALUES (
        v_invoice_id,
        v_item->>'name',
        ...
        COALESCE(v_item->>'serving_type', 'dine_in'),
        COALESCE((v_item->>'packaging_fee')::numeric, 0)
      );
      v_item_insert_count := v_item_insert_count + 1;
    END LOOP;
  END IF;
  v_step3_time := clock_timestamp();

  -- STEP 4: Update batch item statuses (CONCURRENCY GATE)
  IF array_length(p_paid_item_ids, 1) > 0 THEN
  ...
  -- STEP 5: Update batch-level statuses
  ...
  RETURN jsonb_build_object(
    ...
    'item_insert_count', v_item_insert_count,
    'timing_ms', jsonb_build_object(
      'total', EXTRACT(EPOCH FROM (v_step6_time - v_start_time)) * 1000,
      'idempotency', EXTRACT(EPOCH FROM (v_step1_time - v_start_time)) * 1000,
      'invoice', EXTRACT(EPOCH FROM (v_step2_time - v_step1_time)) * 1000,
      'invoice_items', EXTRACT(EPOCH FROM (v_step3_time - v_step2_time)) * 1000,
      'batch_items', EXTRACT(EPOCH FROM (v_step4_time - v_step3_time)) * 1000,
      'batch_status', EXTRACT(EPOCH FROM (v_step5_time - v_step4_time)) * 1000,
      'payment', EXTRACT(EPOCH FROM (v_step6_time - v_step5_time)) * 1000
    )
  );
