...
--   1. Auto-create a session record when the first order batch is placed for a
...
--
-- Why a physical table_ssions table?
...
--     are now paid/cancelled; if so, closes the session (status=closed, records end_time)
--   ▸ `close_table_session(table_id)` RPC can be called manually from the
--     application after payment completes, as a safety net
--
-- Migration file naming convention: YYYYMMDDHHMMSS_descriptive-name.sql
-- ============================================================================
