-- ═══════════════════════════════════════════════════════════════
-- Migration: Remove Tax / VAT / Service Charge Columns
-- ─────────────────────────────────────────────────────────────
-- This migration drops all tax-related columns that were removed
-- from the frontend. The process_payment RPC is also recreated
-- without the p_invoice_tax parameter since the tax column no
-- longer exists.
--
-- Tables affected:
--   public.business_settings  — drops: tax_id, vat_rate,
--     service_charge, tax_inclusive, apply_vat_room_service,
--     apply_service_charge
--   public.invoices           — drops: tax
--
-- RPC affected:
--   public.process_payment    — removes p_invoice_tax parameter
-- ═══════════════════════════════════════════════════════════════

-- ─── 1. Drop columns from business_settings ───────────────────
...
ALTER TABLE public.business_settings
  ...
  DROP COLUMN IF EXISTS apply_vat_room_service,
  DROP COLUMN IF EXISTS apply_service_charge;

-- ─── 2. Drop tax column from invoices ────────────────────────

ALTER TABLE public.invoices
  DROP COLUMN IF EXISTS tax;

-- ─── 3. Recreate process_payment RPC without p_invoice_tax ───
